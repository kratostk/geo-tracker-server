...
const excelize = require("../../utils/excelize");
...
router.get("/fleet/vehicles/:fleetId", isAuth, async (req, res, next) => {
  ...
  console.log("DEBUG : ", typeof fleetId);
  ...
  try {
    ...
    const result = await pool_zbq.query(`
      ...
      SET @FirstDistance = (SELECT TOP 1 b.distance 
            ...
            WHERE a.veh_id = '222150189'
            AND a.local_timestamp BETWEEN '2022-07-04 06:30:00' AND '2022-07-05 06:30:00')
      
      SELECT ISNULL(e.evt_desc, 'LOCATION') AS event 
      ,case 
      ...
      when e.evt_id not in (2,3,23) and isnull(lm.local_timestamp,GETDATE()-30) < GETDATE()-7 then 'OFFLINE'
      else 'NORMAL'
      end as Status
       ...
       , ISNULL((SELECT TOP 1 max_fuel_voltage FROM model WHERE model_id = (SELECT TOP 1 model FROM vehicle WHERE veh_id = '222150189')),600) AS max_fuel_voltage
       , ISNULL((SELECT TOP 1 max_empty_voltage FROM model WHERE model_id = (SELECT TOP 1 model FROM vehicle WHERE veh_id = '222150189')),0) AS max_empty_voltage
       , ISNULL((SELECT TOP 1 max_fuel FROM model WHERE model_id = (SELECT TOP 1 model FROM vehicle WHERE veh_id = '222150189')),60) AS max_fuel
      ...
      WHERE lm.veh_id = '222150189' AND lm.local_timestamp BETWEEN '2022-07-04 06:30:00' AND '2022-07-05 06:30:00'
      `);
...
router.post("/excelize", (req, res, next) => {
  console.log("export excel");
  console.log(path.resolve("image.png"));
  excelize();
});
